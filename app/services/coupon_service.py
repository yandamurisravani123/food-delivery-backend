...
class CouponService:
    ...
    @staticmethod
    async def create_coupon(db: AsyncSession, restaurant_id, coupon_data):
        existing_coupon = await CouponRepository.get_coupon_by_code(db, coupon_data.coupon_code)
        if existing_coupon:
            raise HTTPException(
        ...
        coupon_payload = {**coupon_data.model_dump(), "restaurant_id": restaurant_id}
        return await CouponRepository.create_coupon(db, coupon_payload)

    @staticmethod
    async def validate_coupon(db: AsyncSession, coupon_code: str, order_amount: float):
        coupon = await CouponRepository.get_coupon_by_code(db, coupon_code)
        ...
        if order_amount < coupon.minimum_order_value:
            raise HTTPException(400, f"Minimum order should be {coupon.minimum_order_value}")
