...
async def create_revenue_report(
    ...
    return {"message": "Revenue report created", "report_id": str(report.id)}
...
async def update_revenue_report(
    ...
    await db.commit()
    return {"message": "Revenue report updated"}
...
async def delete_revenue_report(
    ...
    await db.commit()
    return {"message": "Revenue report deleted"}


# REVENUE HISTORY  — specific path must come BEFORE /{restaurant_id}
...
# PAYMENT METHODS  — specific path must come BEFORE /{restaurant_id}
...
# GET REVENUE REPORT  — generic /{restaurant_id} LAST so it doesn't shadow above routes
...
async def get_revenue_report(
    ...
    report = result.scalar_one_or_none()
    if not report:
        return {"message": "No revenue report found"}
